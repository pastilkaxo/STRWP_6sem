...
module.exports = function (request, response) {
  ...
  switch (request.method) {
    ...
    //---------------------DELETE----------------------------
    case "DELETE": {
      const pulpitCode = decodeURIComponent(request.url.split("/")[3]);
      const formattedCode = pulpitCode.padEnd(20, " ");

      Pulpit.destroy({
        where: {
          pulpit: formattedCode,
        },
      })
        .then((result) => {
          if (result === 0) {
            throw new Error("Кафедра не найдена");
          }
          response.end(JSON.stringify({ success: true }));
        })
        .catch((error) => errorHandler(response, 404, error.message));
